...
def parse_arguments():
    ...
    parser.add_argument('--assembly_list', default='',
                        help='comma-separated string of \
                        assembly names to add information, order needs to match \
                            the ordering of the FASTA files as given for Input')

    return parser.parse_args()


def main(fasta1, fasta2, outname, id_name, fasta3='', fasta4='', assembly_list=''):
    ...
    assembly_list = assembly_list.split(',')
    fasta_assembly_dict = dict(zip(fasta_list, assembly_list))
    # create a dictionary that has the sequence as keys, and lists as values of the
    # respective SplitORF IDs
    if assembly_list:
        id_assembly_mapping = defaultdict(str)
    ...
    for fasta in fasta_list:
        if fasta:
            ...
            for record in SeqIO.parse(fasta, "fasta"):
                ...
                if assembly_list:
                    id_assembly_mapping['|'.join(record.id.split(
                        '|')[1:3])] = fasta_assembly_dict[fasta]
                sample_counter += 1
    ...
    # map the assembly information to the index
    if assembly_list:
        so_id_df['assembly'] = so_id_df.index.map(id_assembly_mapping)

    so_name = outname[:-6] + '_so_id_mapping_with_assembly_info.tsv'
...
if __name__ == "__main__":
    ...
    main(fasta1, fasta2, outname, id_name, fasta3, fasta4, assembly_list)
