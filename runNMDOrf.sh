...
if len(sys.argv) < 4:
        print "usage runNMDOrf.sh outputFolder proteins.fa transcripts.fa annotation.bed"
    	print "output Folder will be created if not existing"
else :
	...
	if len(sys.argv) == 5:
		...
		bedtools intersect -a UniqueProteinMatches.bed -b ${annotations} -wa -F 1  -wb > ${output}/intersectResult.txt
