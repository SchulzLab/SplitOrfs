...
python get_nr_ribocov_so_trans_across_all_samples.py \
 ...
 --cancer_cat_csv "${nmd_dir}/SO_coverage_categorization/NMD_cancer/cancer_NMD_interesting_candidates.csv" \
 --region_type NMD


# get the total number of SO transcripts per RI and NMD transcripts that have ribo-cov
python get_nr_ribocov_so_trans_across_all_samples.py \
 --control_cat_csv "${ri_dir}/SO_coverage_categorization/RI_HCT_control/HCT_control_RI_interesting_candidates.csv"\
 --nmd_inh_cat_csv "${ri_dir}/SO_coverage_categorization/RI_NMD_inhibition/NMD_inhibition_RI_interesting_candidates.csv" \
 --cancer_cat_csv "${ri_dir}/SO_coverage_categorization/RI_cancer/cancer_RI_interesting_candidates.csv" \
 --region_type RI
...
if [[ "${pfam}" == true ]]; then 
    bash ../downstream_analysis_validated_URs/check_validated_pfam_domains.sh > ../downstream_analysis_validated_URs/outreports_of_runs/check_validated_pfam_domains_paper_01_08_26.out 2>&1
fi